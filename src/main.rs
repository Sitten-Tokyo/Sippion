...
fn run() -> Result<(), String> {
    ...
    serve_stdio(service).map_err(|error| format!("stdio MCP failed: {error}"))
}
...
fn serve_stdio(service: Arc<dyn RepositoryService>) -> io::Result<()> {
    let stdin = io::stdin();
...
fn handle_request<W: Write>(
    ...
    let result = match method {
        "server/discover" => match validate_bound_protocol(params, true, legacy_initialized) {
            ...
            Ok(ProtocolMode::Legacy) => unreachable!("discover requires modern metadata"),
            Err(error) => Err(error),
        },
...
mod tests {
    ...
    fn modern_requests_do_not_create_connection_protocol_state() {
        ...
        legacy_initialize(&legacy, &initialized)
            .expect("legacy compatibility can initialize later");
        assert!(initialized.load(AtomicOrdering::Acquire));
        assert_eq!(
            validate_bound_protocol(&modern, false, &initialized)
                .expect("modern remains stateless"),
            ProtocolMode::Modern
        );
    }
